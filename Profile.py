...
# already covered a bit of the
# JSON format in class.
#
import json
import time
...
that you should catch in your own code.
...
class Post(dict):
    ...
    for working with individual user
    posts. It currently
    supports two features: A timestamp
    property that is set upon instantiation and
    when the entry object is set and an entry
    property that stores the post message.

    """
    def __init__(self, user: str = None, entry: str = None, timestamp: float = 0, recipient = None):
        ...
        dict.__init__(self, user=self._user, entry=self._entry, timestamp=self._timestamp, recipient=self._recipient)
    ...
    is changed, or set, the timestamp field
        ...
        if p.exists() and p.suffix == '.dsu':
            try:
                ...
                for post_obj in obj['_posts']:
                    ...
                    self._posts.append(post)
                self.friend_list = obj['friend_list']
                self.friend_msg = obj['friend_msg']
                self.your_msg = obj['your_msg']
                f.close()
